...
if exercise_menu == "전체 보기 / 개요" or exercise_menu == "④ 마무리운동":
    ...
    str.success("**🏃 수행 방법**\n\n1.바르게 선 자세에서 양손으로 필라테스 링을 가슴 앞에 잡습니다.\n2.양 무릎을 미세하게 굽힌 상태(Micro-flexion)를 유지하여 부하를 고관절로 분산시킵니다.\n3.코로 숨을 마시고, 입으로 내쉬면서(Exhale) 머리 → 등 → 허리 순서로 마디마디 말아 내려가되, 허리에 통증이 없는 범위(흉추~요추 상부 수준)까지만 하강을 제한합니다.\n4.아래에서 숨을 마시고, 다시 내쉬는 호흡에 양손으로 링을 가볍게 안쪽으로 조이는 힘(등척성 수축)을 주면서 아랫배를 척추 쪽으로 강하게 끌어당겨 척추를 아래부터 하나씩 쌓아 올리며 일어납니다.")
